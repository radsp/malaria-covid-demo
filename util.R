...
# function to get resources from platform permission set
get_countries <- function(user, permissionset, all_resources) {
  path <- paste(
    "/permission_sets",
    permissionset,
    "users",
  ...
  permissions<- civis:::call_api("GET", path, list(), list(), list())
  
  user_countries <- unlist(lapply(permissions,"[",'resourceName'),use.names=FALSE)
  map = setNames(all_countries, all_resources)
  user_countries_mapped <- unlist(lapply(map[user_countries],"["),use.names=FALSE)
  return(user_countries_mapped)
}

extract_countries <- function(dat, region_selected, regions) {
  
  if (region_selected %in% "Northern Hemisphere") {
    yout <- subset(dat, country %in% regions[[2]])
  } else if (region_selected %in% "Southern Hemisphere") {
    yout <- subset(dat, country %in% regions[[3]])
  } else if (region_selected %in% "Western Africa") {
    yout <- subset(dat, country %in% regions[[4]])
  } else if (region_selected %in% "Eastern Africa") { 
    yout <- subset(dat, country %in% regions[[5]])
  } else if (region_selected %in% "Central/Southern Africa") {
    yout <- subset(dat, country %in% regions[[6]])
  } else if (region_selected %in% "Southeast Asia") {
    yout <- subset(dat, country %in% regions[[7]])
  } else if (region_selected %in% "All Countries") {
    yout <- subset(dat, country %in% regions[[1]]) # adding all countries
...
filter_region_snap <- function(dat, region, region_list, regions) {
  
  if (region %in% region_list) {
    yout <- extract_countries(dat, region, regions)
  } else {
...
get_snapshot_data <- function(dat, region, yr0, m0, mf, value_type) {
    ...
    vars <- c("allcause_cases", "confirmed_cases", "tested_cases",
      ...
      if (region %in% region_list) {
        xdat <- extract_countries(dat, region, regions)
...
get_mobplot_snap <- function(dat, m0, mf, yrf, region) {
  ...
  xgmob <- dat %>% 
    ...
    group_by(country, variable, var_label) %>%
    mutate(value = mean(value, na.rm = TRUE)/100) %>%
    ungroup() %>%
...
get_pcnt_ts_data <- function(aggr, region, epiunit, indicator, adm1 = NULL, year0) {
  ...
  yout1 <- extract_countries(yout0, region, regions) %>%
...
get_pcnt_ts_plot <- function(dat, screen_size = NULL) {
  ...
  for (i in gi) {
    ...
    dfi <- subset(dat, (ggroup %in% i) & (str_detect(replace_na(variable, ""), "reports_rate|mob_", negate = TRUE)))
    drr <- subset(dat, (ggroup %in% i) & (str_detect(variable, "reports_rate|mob_")))
...
card <- function(.id, .img, .ctitle, .cdescr, .actBttn) {
  HTML(
    paste0(
      '<div id="', .id, '">
      <div class="card">
      <img src="', .img, '" style="width:100%" class="center">
      <div class="container">
      <h4><i>', .ctitle, '</i></h4>
      <p><div style="max-width:220px;">', .cdescr, '</div></p>
      <p>', .actBttn, '</p>
      </div>
      </div>
      </div>')
