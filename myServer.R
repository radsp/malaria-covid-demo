server <- function(input, output, session){
  ...
  output$out_snapshot_panel <- renderUI({
    ...
    if (snapshot_region() %in% region_list) {
      ...
      fluidPage(style = "margin-left:20px;",
        ...
        fluidRow(
          column(6, leafletOutput("out_map_snapshot")),
          column(5, offset = 1, plotOutput("out_donut_snapshot"))
        ),
        br(),
        br(),
        br(),
    ...
    ### one country view ------------------
      ...
      fluidPage(style = "margin-left:20px",
        ...
        fluidRow(column(6, style = "padding-bottom:5px",
                 ...
                 column(5, offset = 1, style = "padding-bottom:5px",
                        HTML("Placeholder for mobility data plot(map?)"))),
        fluidRow(column(6, htmlOutput("out_txt_maptitle_snapshot"))),
        fluidRow(column(6, leafletOutput("out_map_snapshot")),
                 column(5, offset = 1, plotOutput("out_adm1donut_snapshot"))),
        br(),
        br(),
        # hr(),
        fluidRow(column(12, style = "padding-bottom:2px", 
  ...
  get_mymap <- reactive({
    ...
    if (input$in_region_snapshot %in% region_list) {
      ...
      if(input$in_region_snapshot %in% "Southeast Asia"){
        
      }
      ll0 <- c(19, 4); zlev <- 3
    } else {
      mapdat <- xprov; sdat <- s1
      ll0_tmp <- c(subset(s0, country %in% region_list)[, c("lon_ctr", "lat_ctr")])
      ll0 <- c()
    }
  ...
  #   
  #     xmap0 <- filter_region_snap(xctry, input$in_region_snapshot) %>%
  #       filter(count_type %in% input$in_yaxs_snap)
  #     
  #     mdat0 <- get_shpdat_snap(xmap0, input$in_indicator_snapshot, s0, input$in_yr0_snap,
  #                              input$in_yr0m0_snap, input$in_yr0mf_snap, "covid_cases")
  #     
  #     minval <- min(-100, min(mdat0$delta, na.rm = TRUE), na.rm = TRUE)
  #     maxval <- max(100, max(mdat0$delta, na.rm = TRUE), na.rm = TRUE)
  #     
  #     pal0_allcause <- colorBin(delta_map_clr, domain = mdat0$delta,
  #                               bins = c(minval, -50, -25, -10, 0, 10, 25, 50, maxval))
  #     
  #     mymap <- leaflet(mdat0) %>%
  #       setView(19, 4, zoom = 3) %>%
  #       addProviderTiles("CartoDB.Positron")  %>%
  #       addPolygons(color = "#444444", weight = 1, fillOpacity = 0.8,
  #                   fillColor = ~ pal0_allcause(delta),
  #                   popup = ~htmlEscape(txt_poly)) %>%
  #       addCircleMarkers(lng = ~lon_ctr, lat = ~lat_ctr,
  #                        radius = ~ (covid_cumu_scaled),
  #                        # color = "#ffca05", fill = "#ffca05",
  #                        color = "#a65628", fill = "#a65628",
  #                        fillOpacity = 0.8,
  #                        popup = ~htmlEscape(txt_cov)) %>%
  #       addLegendCustom("bottomleft", colors = "#a65628",
  #                       sizes = 15,
  #                       labels = "COVID-19 Cumulative Cases",
  #                       shapes = "circle",
  #                       borders = "#a65628",
  #                       opacity = 0.96) %>%
  #       addLegend("bottomleft", pal = pal0_allcause, values = ~ delta,
  #                 opacity = 0.8, labFormat = labelFormat(between = " to "),
  #                 title = "% Change")
  #     
  # 
