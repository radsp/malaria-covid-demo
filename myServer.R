server <- function(input, output, session){
  ...
  observeEvent(input$in_plot_type_mal_adm0, ignoreNULL = FALSE, {
    ...
    if(input$in_plot_type_mal_adm0 %in% "Country"){
      ...
      output$out_filter_l2_mal_adm0 <- renderUI(selectInput(inputId = "in_indicator_mal_adm0", label = "Select Indicator:",
                                                            ...
                                                            selected = "All Cause Consultations"))
    } else {
    ...
    get_dat_adm0 <- reactive({
      ...
      if (input$in_plot_type_mal_adm0 %in% "Country") {
        ...
        a <- subset(xctry, country %in% input$in_country_mal_adm0)
                   ...
                   (length(input$in_region_mal_adm0 > 0)) & !(is.null(input$in_region_mal_adm0))) {
  ...
  output$out_plot_mal_adm0 <- renderPlotly({
      ...
      if (!is.null(dat_adm0$dat)) {
        
        if(dat_adm0[["plot_view"]] == "Country"){
          
          g1 <- ggplot(dat_adm0[["dat"]], aes(x = month)) +
            geom_line(aes(x = month, y = value, colour = combogroup, text = info_txt,
                          linetype = combogroup, alpha = combogroup, group = interaction(mygroup, combogroup)), size = 0.5) +
            geom_point(aes(x = month, y = value, colour = combogroup, text = info_txt,
        ...
        if ((dat_adm0$plot_view %in% "Indicator") & (length(unique(dat_adm0$dat$country)) > 12)) {
          pl_height <- 0.7 * ceiling(length(unique(dat_adm0$dat$country)) / 12)
        } else {
          pl_height <- 0.8
        }
  ...
  get_adm1_ctry <- reactive({
    country <- as.character(input$in_country_mal_adm1)
    return(country)
  })
  
  to_listen_adm1 <- reactive({
    list(input$in_plot_type_mal_adm1,input$in_country_mal_adm1)
  })
  
  observeEvent(to_listen_adm1(), ignoreNULL = FALSE, {
    
    if(input$in_plot_type_mal_adm1 %in% "Admin. Level 1"){
      
      ctry_name <- get_adm1_ctry()
      
      prov_name <- unique(as.character(xprov$admin_level_1[xprov$country %in% ctry_name]))
      ...
      output$out_filter_l2_mal_adm1 <- renderUI(selectInput(inputId = "in_indicator_mal_adm1", label = "Select Indicator:",
                                                            choices = c("All Cause Consultations", "Tested Cases", "Malaria Confirmed Cases",
                                                                        "Test Positivity Ratio", "Malaria Severe Cases", "Malaria Deaths", "ANC (1st) Visit"),
                                                            selected = "All Cause Consultations"))
    } else {
  ...
  # Get data for subnational level
  
  
  get_dat_adm1 <- reactive({
    
    if(input$in_plot_type_mal_adm1 %in% "Admin. Level 1"){
      
      a <- subset(xprov, (country %in% input$in_country_mal_adm1) & (admin_level_1 %in% input$in_adm1_mal_adm1) )
      
    } else if (input$in_plot_type_mal_adm1 %in% "Indicator") {
      
      a <- subset(xprov, (boxgroup %in% input$in_indicator_mal_adm1) & (country %in% input$in_country_mal_adm1))
    ...
    if( !(is.null(a)) ){
      ...
      # Exclude Report rate if requested
      if(input$in_radio_include_adm1 == "None") { a <- subset(a, !(combogroup %in% c("Report Rate"))) } 
      
      
      if(input$in_yaxs_mal_adm1 %in% "rate"){
        a <- subset(a, count_type %in% "value_rate")
        ylabel <- "Cases per 1K people"
      } else {
        a <- subset(a, count_type %in% "value")
      ...
      delta <- get_delta(input$in_yr0_mal_adm1, input$in_yr0m0_mal_adm1, input$in_yr0mf_mal_adm1, a)
      delta$text_hover <- paste("Difference between ", input$in_yr0m0_mal_adm1, "-", input$in_yr0mf_mal_adm1, " ", 2020,
                                " and ", input$in_yr0m0_mal_adm1, "-", input$in_yr0mf_mal_adm1, " ", input$in_yr0_mal_adm1, sep = "")
    ...
    m0 <- input$in_yr0m0_mal_adm1
    mf <- input$in_yr0mf_mal_adm1
    yf <- input$in_yr0_mal_adm1
    
    b <- get_ribbon_data(m0, mf, yf, a)
    
    return(list(dat = a, ribbon = b, plot_view = input$in_plot_type_mal_adm1, delta = delta, ylabel = ylabel))
    
  })
  
  
  
  # Plot malaria subnational ---------------------------------------------------------
  
  
  
  output$out_plot_mal_adm1 <- renderPlotly({
      
      dat_adm1 <- get_dat_adm1()
      
      if(dat_adm1[["plot_view"]] == "Admin. Level 1") {
        
        g1 <- ggplot(dat_adm1[["dat"]], aes(x = month)) +
          geom_line(aes(x = month, y = value, colour = combogroup, text = info_txt,
                        linetype = combogroup, alpha = combogroup, group = interaction(mygroup, combogroup)), size = 0.5) +
          geom_point(aes(x = month, y = value, colour = combogroup, text = info_txt,
                         shape = combogroup, size = combogroup, alpha = combogroup, group = interaction(mygroup, combogroup))) +
          geom_ribbon(data = dat_adm1$ribbon, 
                      aes(x = month, ymin = minval, ymax = maxval, fill = combogroup), colour = NA, alpha = 0.1) +
          facet_wrap( ~ boxgroup, scales = "free", ncol = 4) +
          xlab("") + 
          ylab(dat_adm1[["ylabel"]]) +
          ggtitle(as.character(dat_adm1$dat$admin_level_1[1])) +
          scale_x_continuous(breaks = seq(from = 1, to = 12, by = 2), labels = month.abb[seq(from = 1, to = 12, by = 2)]) +
          # geom_text(data = dat_adm0$delta, aes(x = 11, y = 0, label = delta_text, hjust = 1, vjust = -1, text = text_hover), size = 6, colour = "grey20", alpha = 0.75) +
          theme_few(12) +
          scale_alpha_manual("", values = alpha_values, drop = FALSE) +
      ...
      } else if (dat_adm1[["plot_view"]] == "Indicator") {
        
        g1 <- ggplot(dat_adm1$dat) +
          geom_line(aes(x = month, y = value, colour = combogroup, text = info_txt,
                        linetype = combogroup, alpha = combogroup, group = interaction(mygroup, combogroup))) +
          geom_point(aes(x = month, y = value, colour = combogroup, text = info_txt,
                         shape = combogroup, size = combogroup, alpha = combogroup, group = interaction(mygroup, combogroup))) +
          geom_ribbon(data = dat_adm1$ribbon, 
                      aes(x = month, ymin = minval, ymax = maxval, fill = combogroup), colour = NA, alpha = 0.1) +
          facet_wrap( ~ admin_level_1, scales = "free", ncol = 4) +
          # geom_text(data = dat_adm0$delta, aes(x = 11, y = 0, label = delta_text, hjust = 1, vjust = -1, text = text_hover), size = 6, colour = "grey20", alpha = 0.75) +
          xlab("") + 
          ylab(dat_adm1$ylabel) +
      ...
      # To avoid having twenty-something plots in one view
      if ((dat_adm1$plot_view %in% "Indicator") & (length(unique(dat_adm1$dat$admin_level_1)) > 12)) {
        pl_height <- 0.7 * ceiling(length(unique(dat_adm1$dat$admin_level_1)) / 12)
      } else {
        pl_height <- 0.8
      }
...
# })
  ...
  get_dat_rf <- reactive({
    ...
    rfdat_count_type <- input$in_count_type_rf
    
    if (rfdat_adm_level %in% "admin0") {
      tmp <- xrf %>% filter(!(admin_level_1 %in% ""))
      yrf0 <- case_when( rfdat_ctry %in% "All Countries" ~ tmp  %>%  mutate(country = factor(country, levels = ctry_bylat)),
                         rfdat_ctry %in% "Northern Hemisphere" ~ tmp %>% filter(country %in% ctry_nhem) %>% mutate(country = factor(country, levels = ctry_nhem)),
                         rfdat_ctry %in% "Southern Hemisphere" ~ tmp %>% filter(country %in% ctry_shem) %>% mutate(country = factor(country, levels = ctry_shem)),
                         rfdat_ctry %in% "Western Africa" ~ tmp %>% filter(country %in% ctry_wafr) %>% mutate(country = factor(country, levels = ctry_wafr)),
                         rfdat_ctry %in% "Eastern Africa" ~ tmp %>% filter(country %in% ctry_eafr) %>% mutate(country = factor(country, levels = ctry_eafr)),
                         rfdat_ctry %in% "Central/Southern Africa" ~ tmp %>% filter(country %in% ctry_csafr) %>% mutate(country = factor(country, levels = ctry_csafr)),
                         rfdat_ctry %in% "Southeast Asia" ~ tmp %>% filter(country %in% ctry_sea),
                         TRUE ~ tmp %>% filter(country %in% rfdat_ctry))
    } else {
